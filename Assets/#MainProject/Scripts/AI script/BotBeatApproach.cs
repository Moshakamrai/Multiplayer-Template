...
public class BotBeatApproach : NetworkBehaviour
{
    ...
    private IEnumerator RunIn(float secsToBeat)
    {
        ...
        // PLANTED on the attack point. STOP moving: pin exactly on the point and drop the run animation
        // to the idle/combat pose (anim speed back to 1) so the legs aren't still churning. The card's
        // move animation then fires from a standstill on the beat (wind-up/impact drives it), so the bot
        // is clearly stopped while it acts — no sliding through the attack.
        Vector3 plant = ComputeTarget();
        _pc.SetApproachOverride(plant);
        FaceOpponent();
        RpcPlayState(idleState, 0.06f, 1f); // halt the run; the move anim crossfades over this on the beat

        // ── Hold at the player only until the trade outcome arrives (resolves on the beat, so this is
        // usually a frame or two). Short cap so it snaps into knockback/back-dash right after the beat. ──
        float waited = 0f;
        while (_outcome == Outcome.Pending && waited < 0.12f)
        {
